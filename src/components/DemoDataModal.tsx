...
interface Props {
  ...
  onAudit: () => void;
  cloudError?: string;
  onSave: (bundle: DemoBundle) => Promise<'cloud' | 'session'>;
...
export function DemoDataModal(props: Props) {
  ...
  const run = async (items: DemoPackage[]) => {
    ...
    try {
      ...
      setMessage(`${result === 'cloud' ? 'Guardado en Firestore' : 'Vista local activada; se pierde al recargar'} · ${next.expenses.length} gastos · ${next.voters.length} personas ficticias`);
      setPackages([]);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-3" onClick={() => !busy && props.onClose()}>
    <section role="dialog" aria-modal="true" aria-labelledby="demo-data-title" className="max-h-[90dvh] w-full max-w-lg space-y-4 overflow-auto rounded-2xl border border-slate-700 bg-slate-900 p-4 text-slate-200 shadow-2xl" onClick={e => e.stopPropagation()}>
      <header className="flex items-center justify-between gap-2"><div><h2 id="demo-data-title" className="font-bold">Datos · {props.tenantName}</h2><p className="text-xs text-slate-400">{props.realCount ? `${props.realCount} registros reales` : 'Sin registros reales'}{props.bundle ? (props.cloudError ? ' · Demo local' : ' · Demo guardada') : ''}</p></div><button autoFocus disabled={busy} onClick={props.onClose} aria-label="Cerrar datos" className="p-2"><X className="h-5 w-5" /></button></header>
      ...
      {props.cloudError && <div className="rounded-lg border border-amber-700 p-3 text-xs text-amber-200"><p role="status">{props.cloudError}</p><p className="mt-1">Puedes cerrar esta ventana y revisar los gráficos. Esta vista local se pierde al recargar.</p>{props.bundle && <button disabled={disabled} onClick={async () => { setBusy(true); try { const result = await props.onSave(props.bundle!); setMessage(result === 'cloud' ? 'Guardado en Firestore.' : 'Continúa en modo local.'); } catch (e) { setError(e instanceof Error ? e.message : 'No se pudo guardar.'); } finally { setBusy(false); } }} className="mt-2 rounded-lg border border-amber-700 px-3 py-2">Reintentar guardado en nube</button>}</div>}
      {props.bundle && <button disabled={busy} onClick={props.onClose} className="min-h-11 w-full rounded-lg bg-emerald-700 text-sm font-bold">Ver tablero</button>}
